...
class Sender:
    ...
    def modulate(self, message):
        """
        Генерує модульований сигнал за методом FHSS. Результат - 2D-масив (частоти x часові слоти)
        """
        ...
        # 5. PN-код: random зі спільним seed, як індекси частот (0 до num_freq-1)
        random.seed(self.seed)
        pn_indices = np.array([random.randint(0, num_freq - 1) for _ in range(total_length)])
        # 6. Розширення сигналу у відповідності до chip_rate
        repeated_bits = np.repeat(bits, self.chip_rate)
        # 7. Модуляція FHSS: копіюємо синусоїди
        signal = sins.copy()
        # 8. Додаємо модульоване значення тільки до обраної частоти для кожного слота
        signal[pn_indices, np.arange(total_length)] += repeated_bits
        return signal
...
class Receiver:
    ...
    def demodulate(self, signal):
        ...
        # 5. PN-код за тим самим seed, як індекси частот
        random.seed(self.seed)
        pn_indices = np.array([random.randint(0, num_freq - 1) for _ in range(total_length)])
        # 6. Відновлення: витягуємо значення з обраних частот для кожного слота
        recovered_spread = extracted[pn_indices, np.arange(total_length)]
        # 7. "Голосування по більшості": значення конкретних бітів у кожному слоті
        recovered_bits = []
        ...
        # 8. Відновлення текста із послідовності бітів (байтів)
...
if __name__ == '__main__':
    ...
    fhss_message = sender.modulate(message=message)

    receiver = Receiver(
        seed=seed,
        chip_rate=chip_rate
    )
    decoded_message = receiver.demodulate(signal=fhss_message)

    print(f'Original message: {message}')
    print(f'Decoded message: {decoded_message}')
    visualize_signal(fhss_message)